...
procedure TSynAsmSyn.SetLine({$IFDEF FPC}const {$ENDIF}NewValue: String; LineNumber:Integer);
begin
...
function TSynAsmSyn.GetRange: Pointer;
begin
  {$IFDEF SYN_LAZARUS}
...
procedure TSynAsmSyn.SetRange(Value: Pointer);
begin
  {$IFDEF SYN_LAZARUS}
...
procedure TSynAsmSyn.ResetRange;
begin
//  fRange:= rsUnknown;
  {$IFDEF SYN_LAZARUS}
...
procedure TSynAsmSyn.EnumUserSettings(settings: TStrings);
begin
...
function TSynAsmSyn.UseUserSettings(settingIndex: integer): boolean;
begin
...
{$IFNDEF SYN_CPPB_1} class {$ENDIF}
function TSynAsmSyn.GetCapabilities: TSynHighlighterCapabilities;
begin
